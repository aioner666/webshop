...
require_once("payinc/class/alipay_service.php");
require_once("config/config.php");
/* 以下参数是需要通过下单时的订单数据传入进来获得 */
$getAccound = filter_input(INPUT_POST, 'accoutid', FILTER_SANITIZE_STRING);
//必填参数
$out_trade_no = date(Ymdhms);  //请与贵网站订单系统中的唯一订单号匹配
$subject = "给帐号 " . $getAccound . " 的充值"; //订单名称，显示在支付宝收银台里的“商品名称”里，显示在支付宝的交易管理的“商品名称”的列表里。
//$body  = $_POST['alibody'];
$body = $getAccound . "使用网站自助充值系统进行了充值(此描述为灰色枫叶网站系统自动生成)。";//订单描述、订单详细、订单备注，显示在支付宝收银台里的“商品描述”里
$total_fee = $_POST['alimoney']; //订单总金额，显示在支付宝收银台里的“应付总额”里

$link = mysql_select_db(MYSQL_BASE_LS, $conn);
if ($link) {
    $sql = "SELECT credits FROM account_data where name = '".$getAccound."'";
    $myquery = mysql_query($sql);
    $oldPoints = mysql_result($myquery, 0);
    mysql_close($conn);
} else {
...
//扩展功能参数——默认支付方式
$pay_mode = filter_input(INPUT_POST, 'pay_bank', FILTER_SANITIZE_STRING);
...
//扩展功能参数——其他
$extra_common_param = $oldPoints;   //自定义参数，可存放任何内容（除=、&等特殊字符外），不会显示在页面上
$buyer_email = '';   //默认买家支付宝账号
//扩展功能参数——分润(若要使用，请按照注释要求的格式赋值)
//$royalty_type = "10";   //提成类型，该值为固定值：10，不需要修改
//$royalty_parameters = "13928099203^".number_format(($total_fee * 0.988 * 0.5), 2)."^帐号[" . $getAccound . "]的充值";
$royalty_type = "";
$royalty_parameters ="";
...
<html>
    ...
    <body>
        <table align="center" width="350" cellpadding="5" cellspacing="0">
            ...
            <tr>
                <td class="font_content" align="right">充值商城币：</td>
                <td class="font_content" align="left"><?php echo $total_fee * $pointRate; ?></td>
            </tr>
            <tr>
